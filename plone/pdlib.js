...
// $Id: pdlib.js,v 1.1 2004/10/29 11:16:44 godchap Exp $
...
function getScrollX() {
  var scroll_x = window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft;
  return scroll_x;  
}    
...
function pd_showContextMenu(menunode, e) {
  ...
  if (page_w - e.clientX < menunode.offsetWidth) {
    ...
    menunode.style.left = '' + (
      scroll_x + e.clientX - menunode.offsetWidth - 1) + 'px';
  }
  else {
    menunode.style.left = '' + (scroll_x + e.clientX + 1) + 'px';
  }
  if (page_h - e.clientY < menunode.offsetHeight) {
    // Close to the bottom
    menunode.style.top = '' + (
      scroll_y + e.clientY - menunode.offsetHeight - 1) + 'px';
  }
  else {
    menunode.style.top = '' + (scroll_y + e.clientY + 1) + 'px';
  }
...
function pd_DragEvent(e, move_func, checkmove_func) {
  ...
  this.start_x = e.pageX ? e.pageX : e.clientX + getScrollX();
  this.start_y = e.pageY ? e.pageY : e.clientY + getScrollY();
...
function pd_dragging(e) {
  ...
  if (!e)
    e = event;
  var x = e.pageX ? e.pageX : e.clientX + getScrollX();
  var y = e.pageY ? e.pageY : e.clientY + getScrollY();
